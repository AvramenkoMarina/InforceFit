...
T-SHAPED APPS LTD (hereinafter, “Welmi” or the “Provider”), with registered office in Griva Digeni, 51, ATHINEON COURT, Flat/Office 202, 8047 Paphos, Cyprus, operates the website www.welmi.ai (hereinafter, the “Website”) as well as the Welmi app, a digital calorie and health tracker (hereinafter, the “App” or the “Product”) for Android and iOS.

The following Data Protection Policy informs you about the types of personal data of Welmi users that are processed, the purposes for which they are processed, and the scope of processing. The Data Protection Policy applies to all processing of personal data performed by Welmi, both in connection with service provision and, in particular, on the Website and in the Welmi App, which users can install on their mobile end device, as well as within external online presences, such as Welmi’s social media profiles (hereinafter, collectively referred to as the “Online Offer”).
...
3.3. Personal data when using the Welmi App
...
3.3.2. Use of the WELMI app with the feature "Sign Up With Apple" / "Continue With Apple"

The user has the option to create a WELMI account via his or her Apple account. The user can register for or log in to WELMI with this Apple account if he or she uses the "Sign Up With Apple" button on the website or the "Continue With Apple" button in the app when registering. When logging in using the Apple ID, the personal e-mail address can be hidden or shared with WELMI. With “Hide e-mail address”, the user can use Apple's e-mail relay service to generate a generic alias address through which messages from WELMI are forwarded to their private e-mail address. “Continue with Apple" uses two-factor authentication. An additional password does not have to be assigned. Further information about the method of operation and settings is available at: https://support.apple.com/HT210425
...
3.3.3. Use of the WELMI app with the feature "Sign Up With Google" / "Continue With Google"

The user has the option to create a WELMI account via his or her Google account. The user can register for or log in to WELMI with this Google account if he or she uses the "Sign Up With Google" button on the website or the "Continue With Google" button in the app when registering. In this regard, WELMI collects and stores the user's personal data through the Google account, such as the user's e-mail address, optionally also first and last names, and the user's profile picture. The user can individually adjust the data transfer to WELMI in their Google account under privacy settings. Further information about the method of operation and settings is available at: https://support.google.com/accounts/answer/2541991

3.3.4. Use of the WELMI App without personalized user account
...
It is not possible to attribute an anonymous account to a user, meaning that if the mobile end device is lost, it is not possible for WELMI to restore the account.

3.3.5. Data provided by the user

When creating a personalized user account, the user can optionally provide, in addition to the required information (e-mail address and password), first name, last name and place of residence, which are then recorded by WELMI.

In addition, WELMI records data that are provided by the user through a personalized or anonymous user account, which can be entered when using the App. This comprises a user profile, which includes, but is not limited to, the below-listed body and health data:
...
3.3.6. Data automatically recorded by Welmi
...
When the App is being used, WELMI furthermore records the following:

Current IP address
App version being used
Current time zone

These data are recorded in order to improve and personalize the offered services, and this takes place on the basis of WELMI’s legitimate interest pursuant to Article 6(1)(f) GDPR.

3.3.8. Contractual relationship and payment procedure

Where a contractual relationship between the user and WELMI is to be established, substantively structured or changed, WELMI stores personal data of the user on the basis of Article 6(1)(b) GDPR (see the list of personal data under No. 3.3.5 to 3.3.7), provided this is necessary for the performance of the contract.

Through an in-app purchase, the user has the ability to acquire a PRO version of the WELMI App in connection with a subscription, optionally at the start of a seven-day free trial phase (trial subscription). If the user decides to acquire the PRO version and presses the order button, they will be forwarded directly to the Apple App Store, the Google Play Store, depending on their operating system, even if they had first opted for the seven-day free trial phase during the temporary subscription. In that case, the corresponding amount for the subscription will be automatically debited after the seventh day of the trial phase.

When the user is forwarded to the appropriate app store, WELMI transmits the starting and ending date and, if applicable, the subscription termination date, as well as the reason for termination (e.g., after a possible cancellation). The payment processing data are collected directly by the app stores.
...
The PRO version of the App can also be purchased through the WELMI Website. Depending on the selected payment option (Apple Pay, Google Pay, SEPA payment or credit card payment), the user must provide the relevant account data, even if they are first using the seven-day trial subscription.
...
Further processing is handled by the payment service provider Stripe. The service is offered by Stripe Payments Europe Ltd, Block 4, Harcourt Centre, Harcourt Road, Dublin 2, Ireland. Information provided by the user (name, e-mail address, credit card data, start and end date, and, if applicable, termination data of subscriptions, invoice amount) is transmitted by WELMI to Stripe pursuant to Article 6(1)(b) GDPR as part of the order process. User data are shared solely for the purpose of payment processing with the payment service provider Stripe Payments Europe Ltd. and only to the extent they are necessary for this. Further information about data protection by Stripe is available at https://stripe.com/legal.

Moreover, WELMI makes the mobile payment system Apple Pay of Apple Inc., One Infinite Loop, 95014 Cupertino, USA, available to users in the WELMI App. When Apple Pay is used, Apple does not store any original credit card, debit card or prepaid card numbers used by the user. Apple has no access to them. In addition, no backup of transaction data takes place that permits conclusions to be drawn as to the user. Further data protection notices can be found at https://www.apple.com/privacy/.
...
Welmi offers an optional feature that provides AI-generated wellness and nutrition reports (“AI Health Reports”). These reports are created based on information voluntarily provided by the user, such as responses to questionnaires, dietary habits, lifestyle details, and other inputs.

Important Disclaimer:

AI Health Reports do not constitute medical advice, diagnosis, treatment, or professional healthcare services. Welmi is not a medical provider, does not perform medical evaluations, and does not offer emergency or crisis support. Any insights, assessments, or recommendations generated by the AI are intended for informational and educational purposes only.
...
To generate AI Health Reports, Welmi may process the following categories of data:
...
— develop and enhance Welmi’s non-medical wellness features.
...
WELMI takes the protection of personal user data very seriously. For this reason, WELMI treats personal data confidentially and in accordance with statutory provisions concerning data protection as well as this Data Protection Policy. WELMI therefore collects and stores only data that are provided by third-party providers on the basis of the respective user consent under Article 6(1)(a) GDPR and transmits corresponding data to them.

Subject to statutory or contractual permissions, WELMI processes the data in a third country or has the data processed there only where the special requirements of Articles 44 et seqq. GDPR are met. Processing takes place, e.g., on the basis of special safeguards, such as the observance of officially recognized, special contractual obligations (“standard contract clauses”).

For data transfers to the USA, WELMI primarily relies on the EU-U.S. Trans-Atlantic Data Privacy Framework (DPF), which has been recognised as a secure legal framework by an adequacy decision of the European Commission. Standard contractual clauses may also exist. Further information on the DPF and a list of certified companies can be found (in English) at https://www.dataprivacyframework.gov/.

If the European Commission does not attribute to a third country the same level of data protection as the EU, then WELMI ensures the maintenance of the European level of data protection through the use of standard contract clauses (SCC) and binding company rules pursuant to Article 46(1) and (2)(c) GDPR. Nevertheless, it is possible under certain circumstances that authorities in a third country may access user data for control and monitoring purposes and, in this regard, that effective legal remedies and data subject rights may not be enforceable.

The following providers and data are affected:

4.1. Google Fit

WELMI offers its users the option to link the App with Google Fit, a health platform of Google Ireland Limited, Gordon House, Barrow Street, Dublin 4, Ireland (“Google”). Google Fit logs the physical activities of the respective user using sensors of the mobile device or activity sensors. The following data may be transmitted in the case of a linking with WELMI:
...
The use of information that WELMI obtains from Google APIs and the sharing of it with other apps takes place pursuant to Google API Services-User Data Policy, including the requirements for limited use.

4.2 Google Health Connect

WELMI offers its users the option to link the app with Google Health Connect, an integrated health data platform provided by Google Ireland Limited, Gordon House, Barrow Street, Dublin 4, Ireland (“Google”). Google Health Connect facilitates the consolidation of health and wellness data across multiple applications.

When users link WELMI with Health Connect, the following categories of data may be accessed or shared:
...
This integration allows users to enhance their experience within the WELMI app by providing more comprehensive health insights and personalized recommendations based on aggregated data.

WELMI adheres to the Health Connect Permissions Policy, including Limited Use requirements, ensuring that any data obtained is solely utilized for the enhancement of user features within the app. Users maintain full control over their data and can modify their permissions or unlink Health Connect at any time through the WELMI app settings, being informed of any potential impacts on app functionality.
...
Users also have the option to link their App with Apple Health, a mobile app of the technology group Apple Inc., One Infinite Loop, 95014 Cupertino, USA. The below-listed data may be transmitted to WELMI in the course of this:
...
In addition, the below-listed data may be transmitted by WELMI to Apple:
...
WELMI has the ability to directly access the data pool of Apple Health at any time if the user has consented to this. Apple Health collects health-relevant user data and displays them as processed in the App. Further data protection notices can be found at https://www.apple.com/privacy/.

4.4. Samsung Health

Furthermore, health, diet and fitness data of users of Samsung Health, a platform of Samsung Electronics, Ltd, 129, Samsung-ro, Yeongtong-gu, Suwon-si, Gyeonggi-do 16677, Republic of Korea, and Samsung Medison, Pangyogeok-ro, 145, Bundang-gu, Republic of Korea, can be transmitted to WELMI if the user uses the connect option in the WELMI App:
...
Samsung Health manages the mentioned user data in order to support its users on the path to greater fitness and well-being. WELMI has the ability to directly access the data pool of Samsung at any time. Further information is available at https://privacy.samsung.com/privacy/samsung.

4.5. Huawei Health

Health and fitness data of users can be transmitted by Huawei Health, an official app of Huawei, G1, HUAWEI Industrial Base, Bantian, Longgang District, Shenzhen 518129 P.R. China, to WELMI if the user uses the connect option in the WELMI App:
...
WELMI Health monitors the health and physical activities of users. WELMI has the ability to directly access the data pool of Huawei at any time. Further data protection notices are available at:

https://consumer.huawei.com/minisite/cloudservice/health/privacy-statement.htm?code=HK&language=en_us.

5. Contact & Customer Support

Welmi uses a customer service platform to process user inquiries submitted through the Website. The Website provides a contact form in which users manually enter their data (name, email address, iOS or Android version, possible PRO status, description of the issue). Based on this information, the Welmi Support Team can view the user data stored in the user’s App.

The personal data that users provide to Welmi in connection with their contact request is used solely to respond to the inquiry or to establish contact and is processed only for the technical administration associated with this request. The data is not shared with third parties.

If the user has consented to the storage of their data, they have the right to withdraw this consent at any time with effect for the future. In such cases, their personal data will be deleted without delay. The user’s personal data will also be deleted without a separate request if Welmi has completed processing of their inquiry or if storage is not permissible for other legal reasons.
...
WELMI regularly notifies its users by e-mail about current diet trends, recipes and other interesting offers and tips from the field of nutrition, weight loss, etc. Registration is voluntary and takes place using a double opt-in procedure. After registering, the user receives an e-mail requesting that they confirm their registration. This is necessary so that a third party cannot register to receive the newsletter using someone else's e-mail address. In order to document that the registration meets the legal requirements, it is logged on the basis of WELMI's legitimate interest pursuant to Article 6(1)(f) GDPR.

As part of registration on the Website or in the App, the user consents to the processing of the provided data for the purpose of sending and receiving e-mails (Article 6(1)(a) GDPR). Furthermore, the user consents that WELMI may collect and process data about their usage behavior (namely, opening the e-mail and clicking on links in it) so that the content of the mailings can be tailored to the respective needs, e.g., if a user repeatedly clicks on links about topic A but not about topic B, they will receive only links about topic A in future mailings.

The consent granted by the user to receive these emails may be withdrawn at any time and without providing reasons with prospective effect by sending an e-mail to support@welmi.ai.

In addition, receipt can be stopped by using a link found at the end of each newsletter. Welmi may store the provided e-mail address and the data stored in connection with logging the registration for up to three years on the basis of its legitimate interest in order to be able to demonstrate that consent had previously been given. The processing of these data is limited to the purpose of possible defense against claims. An individual erasure request is possible at any time, provided that the former existence of consent is confirmed at the same time.

7. Use by Children

WELMI is aware of the importance of additional measures to protect the privacy of children. Persons under the age of 16 may not open an account unless a parent has consented in accordance with applicable law. Should WELMI learn that personal data about a child under the minimum age were collected without parental consent, WELMI will initiate steps to erase these data without delay. Parents who believe that their child has sent us personal data and would like to have these erased should contact WELMI using the contact data provided in No. 1.

8. Use of Tools for Web analysis, Remarketing & Retargeting

Based on the user's consent within the meaning of Article 6(1)(a) GDPR, WELMI utilizes various tools and plug-ins for the purpose of web analysis, remarketing and retargeting. In doing so, cookies are used that forward the IP address and/or record and analyze different types of data. This includes, for example, the number of Website visitors, visit duration, average page-loading time and visitor origin. These cookies are used for the purpose of being able to put together more targeted offers for WELMI users.

Specifically:

8.1. Google Tag Manager/Google Analytics/Google Optimize/Google Analytics by Firebase

WELMI utilizes Google Tag Manager on its Website. The provider is Google Ireland Ltd. (“Google”), Google Building Gordon House, Barrow Street, Dublin 4, Ireland. Google Tag Manager is a tool used to embed tracking or statistics tools and other technologies on the Website. Google Tag Manager does not itself create any user profiles, store any cookies, or perform any independent analyses. It is merely used to manage and run the tools embedded through it. For this purpose, however, Google Tag Manager records certain aggregated data for the purpose of diagnosing how the tracking and statistics tools are running. These data do not contain any IP addresses or IDs that can be linked to a specific person.

WELMI uses Google Analytics and Google Optimize on the Website and in the App for the purpose of analyzing use of the Website. The provider of both web analysis services is Google Ireland Ltd. (“Google”), Google Building Gordon House, Barrow Street, Dublin 4, Ireland.
...
On the Website and in the App, the Provider uses Google Ads Remarketing, an online advertising program of Google Ireland Ltd. (“Google”), Google Building Gordon House, Barrow Street, Dublin 4, Ireland. In connection with Google Ads Remarketing, WELMI uses what is known as conversion tracking. If the user clicks on an advertisement placed by Google, a cookie is set for the conversion tracking. These cookies lose their validity after 30 days and are not used to personally identify users. If the user visits certain pages on this Website and the cookie has not yet expired, Google and the Provider can recognize that the user clicked on the advertisement and was redirected to this site.
...
WELMI utilizes Apple Search Ads in the App. Apple Search Ads is an online advertising program of the technology group Apple, One Infinite Loop, 95014 Cupertino, USA. Through Apple Search Ads, the Provider's App will be displayed to the user as the top result when the user searches in the App Store.
...
WELMI stores the user's personal data for the period of use of the App. If the user account is deleted, the e-mail address, first name, last name, profile picture and links to third-party providers will be definitively and irretrievably deleted.
...
The user has the right at any time to obtain free information about their personal data stored by WELMI, the origin and recipients of the data, the purpose of data processing and the planned duration of data storage, including a copy of the personal data that are the subject of the processing.
...
Subject to the prerequisites of Article 17 GDPR, the user may request erasure of their personal data. Their entitlement to erasure depends, inter alia, on whether the data concerning them are still needed by WELMI for fulfilling its statutory duties.
...
8047, Paphos, Cyprus
Email: support@welmi.ai
Web: www.welmi.ai

12. Version & Updating of This Data Protection Policy

This Data Protection Policy is currently valid in the version of June 2025.

If our Website or our Product is enhanced, or if legal requirements should change, it may become necessary to amend this Data Protection Policy. The current version of this Data Protection Policy can be viewed and printed out at any time by visiting https://www.welmi.ai/policy`;

  const lines = policyText.split("\n");

  const blocks: Array<{
    type: "heading" | "subheading" | "paragraph" | "list" | "address";
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line === "T-SHAPED APPS LTD") {
      flushList();

      const addressLines: string[] = [line];
      for (let j = i + 1; j < lines.length; j++) {
        const next = lines[j].trim();

        if (!next || /^\d+\.\s+/.test(next) || /^\d+\.\d+/.test(next)) {
          i = j - 1;
          break;
        }
        ...
        if (
          next === "support@welmi.ai" ||
          next.startsWith("Email:") ||
          next.startsWith("Web:")
        ) {
          i = j;
          break;
      ...
      blocks.push({ type: "address", content: addressLines });
      continue;
    }
    ...
    if (/^\d+\.\s+/.test(line)) {
      blocks.push({ type: "heading", content: line.toUpperCase() });
      continue;
    }
